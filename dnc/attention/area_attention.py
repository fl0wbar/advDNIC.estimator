...
def lengths_to_area_mask(feature_length, length, max_area_size):
    ...
    paddings = tf.cast(
        tf.expand_dims(
            tf.logical_not(tf.sequence_mask(feature_length, maxlen=length)), 2
        ),
        tf.float32,
    )
...
def basic_pool(
    features, max_area_width, max_area_height=1, height=1, fn=tf.reduce_max, name=None
):
    ...
    with tf.name_scope(name, default_name="basic_pool"):
        ...
        for area_height in range(max_area_height):
            for area_width in range(max_area_width):
                ...
                pool_list.append(tf.reshape(pool_tensor, [batch_size, -1, depth]))
                height_list.append(
                    tf.reshape(
                        size_tensor[:, area_height:, area_width:] * (area_height + 1),
                        [batch_size, -1],
                    )
                )
                width_list.append(
                    tf.reshape(
                        size_tensor[:, area_height:, area_width:] * (area_width + 1),
                        [batch_size, -1],
                    )
                )
...
def _compute_sum_image(
    features, max_area_width, max_area_height=1, height=1, name=None
):
...
def compute_area_features(
    features, max_area_width, max_area_height=1, height=1, epsilon=1e-6
):
...
def compute_area_key(
    ...
    area_mean, area_std, _, area_heights, area_widths = compute_area_features(
        features,
        max_area_width=max_area_width,
        max_area_height=max_area_height,
        height=height,
    ...
    with tf.variable_scope(
        ...
        height_embed = tf.nn.embedding_lookup(
            params=tf.get_variable("area_height_emb", [max_area_height, depth // 2]),
            ids=area_heights[:, :, 0] - 1,
        )
        width_embed = tf.nn.embedding_lookup(
            params=tf.get_variable("area_width_emb", [max_area_width, depth // 2]),
            ids=area_widths[:, :, 0] - 1,
        )
        ...
        feature_hidden = tf.layers.dense(
            inputs=feature_concat, units=depth, activation=tf.nn.relu
        )
        area_key = tf.layers.dense(feature_hidden, units=depth)
        return area_key


def dot_product_area_attention(
    q,
    k,
    v,
    bias,
    ...
    with tf.variable_scope(
        name, default_name="dot_product_area_attention", values=[q, k, v]
    ) as scope:
        ...
        if top_k_areas > 0:
            ...
            weights = tf.where(
                tf.greater_equal(weights, min_values), weights, tf.zeros_like(weights)
            )
