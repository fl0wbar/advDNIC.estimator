...
from dnc.attention import common_attention
...
class DistributionType(object):
    ...
    @staticmethod
    def get_choices():
        return [DistributionType.CAT, DistributionType.DMOL]
...
def local_within_block_attention(
    x,
    self_attention_bias,
    hparams,
...
def local_attention_1d(
    x, hparams, attention_type="local_unmasked", q_padding="VALID", kv_padding="VALID"
):
...
def get_dilated_1d_attention_mask(
    num_heads, block_size, num_blocks, memory_size, gap_size, name="dilated_mask"
):
...
def local_global_attention(
    x, self_attention_bias, hparams, q_padding="LEFT", kv_padding="LEFT"
):
...
def encdec_attention_1d(x, encoder_output, encoder_decoder_attention_bias, hparams):
    """Local 1d self attention."""
    x, x_shape, is_4d = maybe_reshape_4d_to_3d(x)
...
def transformer_decoder_layers(
    ...
    for layer in range(num_layers):
        with tf.variable_scope("%s_layer_%d" % (name, layer)):
            ...
            y = ffn_layer(
                common_layers.layer_preprocess(x, hparams), hparams, losses=losses
            )
            x = common_layers.layer_postprocess(x, y, hparams)
...
def postprocess_image(x, rows, cols, hparams):
    ...
    if likelihood == DistributionType.DMOL:
        ...
        targets = tf.layers.dense(
            x, depth, use_bias=False, activation=None, name="output_conv"
        )
    else:
        depth = 256
        targets = tf.layers.dense(
            x, depth, use_bias=True, activation=None, name="output_conv"
        )
    if hparams.mode == tf.estimator.ModeKeys.PREDICT and hparams.block_raster_scan:
        ...
        y = tf.reshape(
            y, [batch, yshape[1] // block_length, block_length, yshape[2], depth]
        )
        yshape = common_layers.shape_list(y)
        # Break into blocks width wise.
        y_blocks = tf.reshape(
            y,
            [batch, yshape[1], yshape[2], yshape[3] // block_width, block_width, depth],
        )
    ...
    if hparams.mode == tf.estimator.ModeKeys.PREDICT:
        ...
        if hparams.block_raster_scan:
            ...
            targets = tf.pad(
                ...
                [
                    [0, 0],
                    [0, -curr_infer_length % block_padding_factor],
                    [0, 0],
                    [0, 0],
                ],
            )
        ...
        targets = tf.reshape(targets, [targets_shape[0], -1, hparams.img_len, channels])
    # Preprocess image
    x = prepare_image(targets, hparams, name="dec_channels")
    x_shape = common_layers.shape_list(x)
    if (
        hparams.dec_attention_type == AttentionType.LOCAL_2D
        or hparams.dec_attention_type == AttentionType.LOCAL_BLOCK
    ):
...
def create_output(decoder_output, rows, cols, targets, hparams):
    ...
    if hparams.mode == tf.estimator.ModeKeys.PREDICT:
        ...
        channels = hparams.num_channels
        output = tf.reshape(
            decoded_image, [batch, rows, cols // channels, channels, depth]
        )
    else:
...
def get_channel_embeddings(io_depth, targets, hidden_size, name="channel"):
    ...
    rgb_embedding_var = tf.get_variable(
        "rgb_target_emb_%s" % name, [256 * io_depth, hidden_size]
    )
