...
class SubmitConfig(util.EasyDict):
    ...
    Attributes:
        ...
        run_dir_ignore: List of file patterns used to ignore files when copying files
        to the run dir.
        ...
        submit_target: Submit target enum value. Used to select where the run is
        actually launched.
        ...
        user_name: Automatically populated value during submit. Can be set by the user
        which will then override the automatic value.
...
def convert_path(path: str, path_type: PathType = PathType.AUTO) -> str:
    """Convert a normal path to template and the convert it back to a normal path with
    given path type."""
...
def _get_next_run_id_local(run_dir_root: str) -> int:
    """Reads all directory names in a given directory (non-recursive) and returns the
    next (increasing) run id. Assumes IDs are numbers at the start of the directory
    names."""
    dir_names = [
        d
...
def _populate_run_dir(run_dir: str, submit_config: SubmitConfig) -> None:
    """Copy all necessary files into the run dir. Assumes that the dir exists,
    is local, and is writable."""
...
def run_wrapper(submit_config: SubmitConfig) -> None:
    ...
    # when running locally, redirect stderr to stdout, log stdout to a file, and force
    # flushing
    if is_local:
        logger = util.Logger(
    ...
    else:  # when running in a cluster, redirect stderr to stdout, and just force
        # flushing (log writing is handled by run.sh)
        logger = util.Logger(file_name=None, should_flush=True)

    import dnc.dnnlib as dnnlib
...
def submit_run(
    ...
    """Create a run dir, gather files related to the run, copy files to the run dir,
