...
def kl_divergence(mu, log_var, mu_p=0.0, log_var_p=0.0):
    ...
    kld = tfp.distributions.kl_divergence(posterior_distribution, prior_distribution)
...
def cumsum(x, axis=0, exclusive=False):
    ...
    mask = tf.cast(
        comparator(tf.expand_dims(my_range, 1), tf.expand_dims(my_range, 0)), x.dtype
    )
...
def deconv_stride2_multistep(x, nbr_steps, output_filters, name=None, reuse=None):
    ...
    with tf.variable_scope(
        ...
        def deconv1d(cur, i):
            ...
            return tf.reshape(
                thicker, [cur_shape[0], cur_shape[1] * 2, 1, output_filters]
            )
...
def conv_internal(conv_fn, inputs, filters, kernel_size, **kwargs):
    ...
    if kwargs.get("padding") == "LEFT":
        ...
        cond_padding = tf.cond(
            tf.equal(shape_list(inputs)[2], 1),
...
def subseparable_conv(inputs, filters, kernel_size, **kwargs):
    ...
    def conv_fn(inputs, filters, kernel_size, **kwargs):
        ...
        if separability:
            ...
            for split_idx, split in enumerate(tf.split(inputs, abs_sep, axis=3)):
                with tf.variable_scope("part_%d" % split_idx):
                    if separability > 0:
                        parts.append(
                            layers().Conv2D(
                                filters // separability, kernel_size, **kwargs
                            )(split)
                        )
                    else:
                        parts.append(
                            layers().SeparableConv2D(
                                filters // abs_sep, kernel_size, **kwargs
                            )(split)
                        )
            if separability > 1:
            ...
            result = layers().SeparableConv2D(filters, kernel_size, **kwargs)(inputs)
...
def conv_block(inputs, filters, dilation_rates_and_kernel_sizes, **kwargs):
    ...
    return conv_block_internal(
        conv, inputs, filters, dilation_rates_and_kernel_sizes, **kwargs
    )
...
def conv1d_block(inputs, filters, dilation_rates_and_kernel_sizes, **kwargs):
    ...
    return conv_block_internal(
        conv1d, inputs, filters, dilation_rates_and_kernel_sizes, **kwargs
    )
...
def separable_conv_block(inputs, filters, dilation_rates_and_kernel_sizes, **kwargs):
    ...
    return conv_block_internal(
        separable_conv, inputs, filters, dilation_rates_and_kernel_sizes, **kwargs
    )
...
def subseparable_conv_block(inputs, filters, dilation_rates_and_kernel_sizes, **kwargs):
    """A block of separable convolutions."""
    return conv_block_internal(
        subseparable_conv, inputs, filters, dilation_rates_and_kernel_sizes, **kwargs
    )


def conv_block_downsample(
    x, kernel, strides, padding, separability=0, name=None, reuse=None
):
...
def upscale(inputs, f, method=tf.image.ResizeMethod.NEAREST_NEIGHBOR):
    ...
    height, width = shape_list(inputs)[
        1:3
    ...
    with tf.variable_scope("upconv"):
        ...
        if method == "nn_upsample_conv":
            ...
            net = layers().Conv2DTranspose(
                num_outputs, (3, 3), strides=stride, activation=tf.nn.relu
            )(net)
            net = net[:, 1:, 1:, :]
        else:
...
def td_conv(
    ...
    with tf.variable_scope(name, default_name="td_conv", reuse=reuse):
        ...
        w = tf.get_variable("DW", shape=kernel_shape, initializer=kernel_initializer)
        if use_bias:
...
def targeted_dropout(inputs, k, keep_prob, targeting_fn, is_training, do_prune=False):
    ...
    if not is_training and do_prune:
        k = tf.round(to_float(k) * to_float(1.0 - keep_prob))
...
def dense_dropconnect(
    inputs, output_size, dropconnect_dropout=0.0, name="dense_dropconnect", **kwargs
):
...
def conv_gru(
    x, kernel_size, filters, padding="SAME", dilation_rate=(1, 1), name=None, reuse=None
):
...
def conv_lstm(
    ...
    with tf.variable_scope(name, default_name="conv_lstm", values=[x], reuse=reuse):
        gates = conv(
            x, 4 * filters, kernel_size, padding=padding, dilation_rate=dilation_rate
        )
...
def pad_to_same_length(x, y, final_length_divisible_by=1, axis=1):
    ...
    with tf.name_scope("pad_to_same_length", values=[x, y]):
        ...
        if (
            isinstance(x_length, int)
            and isinstance(y_length, int)
            and x_length == y_length
            and final_length_divisible_by == 1
        ):
            return x, y
...
def weights_multi_problem_all(labels, taskid=-1):
    ...
    example_mask = to_float(tf.greater(example_mask, tf.zeros_like(example_mask)))
...
def _weights_one_third(labels):
    ...
    return tf.ones(tf.shape(labels)[:-1]) / 3.0
...
def split_to_discretized_mix_logistic_params(inputs):
    ...
    """
    batch, height, width, output_dim = shape_list(
        inputs
    )  # pylint: disable=unbalanced-tuple-unpacking
...
def discretized_mix_logistic_loss(pred, labels):
    ...
    batch, height, width, num_mixtures = shape_list(
        logits
    )  # pylint: disable=unbalanced-tuple-unpacking
    labels = tf.tile(
        tf.reshape(labels, [batch, height, width, 1, 3]), [1, 1, 1, num_mixtures, 1]
    )
    ...
    means_2 = (
        locs[..., 2] + coeffs[..., 1] * labels[..., 0] + coeffs[..., 2] * labels[..., 1]
    )
    means = tf.stack([means_0, means_1, means_2], axis=-1)
    centered_labels = labels - means
    inv_stdv = tf.exp(-log_scales)
    plus_in = inv_stdv * (centered_labels + 1.0 / 255.0)
    min_in = inv_stdv * (centered_labels - 1.0 / 255.0)
    ...
    log_prob_event_approx = (
        mid_in - log_scales - 2.0 * tf.nn.softplus(mid_in) - np.log(127.5)
    )
    log_probs = tf.where(
        labels < -0.999,
        log_prob_0,
...
def sample_from_discretized_mix_logistic(pred, seed=None):
    ...
    gumbel_noise = -tf.log(
        -tf.log(
            tf.random_uniform(
                tf.shape(logits), minval=1e-5, maxval=1.0 - 1e-5, seed=seed
            )
        )
    )
    sel = tf.one_hot(
        tf.argmax(logits + gumbel_noise, -1), depth=num_mixtures, dtype=tf.float32
    )
    ...
    uniform_noise = tf.random_uniform(
        tf.shape(locs), minval=1e-5, maxval=1.0 - 1e-5, seed=seed
    )
...
def smoothing_cross_entropy(logits, labels, vocab_size, confidence, gaussian=False):
    ...
    with tf.name_scope("smoothing_cross_entropy", values=[logits, labels]):
        ...
        normalizing = -(
            ...
            + to_float(vocab_size - 1) * low_confidence * tf.log(low_confidence + 1e-20)
        )
...
def sru_with_scan(
    x, num_layers=2, activation=None, initial_state=None, name=None, reuse=None
):
...
def sru(x, num_layers=2, activation=None, initial_state=None, name=None, reuse=None):
    """SRU cell as in https://arxiv.org/abs/1709.02755.
    ...
    try:
        from tensorflow.contrib.recurrent.python.ops import (
            functional_rnn,
    ...
    with tf.variable_scope(name, default_name="sru", values=[x], reuse=reuse):
        ...
        for i in range(num_layers):
            ...
            c_states, _ = functional_rnn.functional_rnn(cell, concat, time_major=False)
            # Final output.
            if activation is not None:
                c_states = activation(c_states)
            h = c_states * r + (1.0 - r) * x_orig
...
def linear_set_layer(
    layer_size, inputs, context=None, activation_fn=tf.nn.relu, dropout=0.0, name=None
):
...
tf.register_tensor_conversion_function(
    FactoredTensor, _convert_factored_tensor_to_tensor
)
...
def conv_hidden_relu_memory_efficient(
    x, filter_size, epsilon=1e-6, forget=True, test_vars=None, name=None
):
...
def layer_norm(
    x, filters=None, epsilon=1e-6, name=None, reuse=None, layer_collection=None
):
...
def sample_with_temperature(logits, temperature, sampling_keep_top_k=-1):
    ...
    if temperature == 0.0:
        ...
        if sampling_keep_top_k != -1:
            ...
            logits = tf.where(
                tf.less_equal(logits, k_largest), tf.ones_like(logits) * -1e6, logits
            )

        reshaped_logits = tf.reshape(logits, [-1, shape_list(logits)[-1]]) / temperature
        choices = tf.multinomial(reshaped_logits, 1)
        choices = tf.reshape(
            choices, shape_list(logits)[: logits.get_shape().ndims - 1]
        )
        return choices
...
def dense(x, units, **kwargs):
    ...
    if layer_collection:
        ...
        layer_params = tf.get_collection(
            tf.GraphKeys.GLOBAL_VARIABLES, scope=layer_name
        )
        assert layer_params
        ...
        tf.logging.info(
            "Registering dense layer to collection for tensor: {}".format(layer_params)
        )
...
def batch_dense(
    inputs, units, activation=None, kernel_initializer=None, reuse=None, name=None
):
    ...
    Args:
        ...
        reuse: whether to reuse the variable scope
        name: an optional string
    Returns:
...
def argmax_with_score(logits, axis=None):
    ...
    flat_indices = tf.stack(
        [tf.range(tf.to_int64(prefix_size)), tf.to_int64(flat_predictions)], axis=1
    )
...
def index_last_dim_with_indices(x, indices):
    ...
    idx = tf.stack(
        [tf.range(tf.to_int64(shape_list(flat_indices)[0])), tf.to_int64(flat_indices)],
        axis=1,
    )
...
def deep_discriminator(
    x, batch_norm, is_training, filters=64, filter_size=4, stride=2, output_size=1024
):
    """Discriminator architecture based on InfoGAN."""
    with tf.variable_scope(
        "discriminator", initializer=tf.random_normal_initializer(stddev=0.02)
    ):
        batch_size, height, width = shape_list(x)[
            :3
        ]  # pylint: disable=unbalanced-tuple-unpacking
        net = layers().Conv2D(
            filters, filter_size, strides=stride, padding="SAME", name="conv1"
        )(x)
        net = lrelu(net)
        net = layers().Conv2D(
            2 * filters, filter_size, strides=stride, padding="SAME", name="conv2"
        )(net)
        # [bs, h/4, w/4, 128]
        if batch_norm:
            net = layers().BatchNormalization(
                training=is_training, momentum=0.999, name="d_bn2"
            )(net)
        net = lrelu(net)
        size = height * width
        x_shape = x.get_shape().as_list()
        ...
        if batch_norm:
            net = layers().BatchNormalization(
                training=is_training, momentum=0.999, name="d_bn3"
            )(net)
        net = lrelu(net)
        return net
...
def double_discriminator(
    x, filters1=128, filters2=None, kernel_size=8, strides=4, pure_mean=False
):
    ...
    with tf.variable_scope("discriminator"):
        batch_size = shape_list(x)[0]
        net = layers().Conv2D(
            filters1, kernel_size, strides=strides, padding="SAME", name="conv1"
        )(x)
        if pure_mean:
        ...
        net = tf.nn.relu(net)
        net = layers().Conv2D(
            filters2, kernel_size, strides=strides, padding="SAME", name="conv2"
        )(net)
        if pure_mean:
