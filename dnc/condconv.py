...
def get_condconv_initializer(initializer, num_experts, expert_shape):
    ...
    def condconv_initializer(expected_shape, dtype=None, partition=None):
        ...
        if (
            ...
            or expected_shape[0] != num_experts
            or expected_shape[1] != num_params
        ):
            raise (
                ValueError(
...
class CondConv2D(tf.keras.layers.Conv2D):
    ...
    def build(self, input_shape):
        if len(input_shape) != 4:
            raise ValueError(
                "Inputs to `CondConv2D` should have rank 4. " "Received input shape:",
                str(input_shape),
            )
        ...
        if self.use_bias:
            ...
            for output, bias in zip(outputs, biases):
                ...
                bias_outputs_list.append(
                    tf.nn.bias_add(output, bias, data_format=self.converted_data_format)
                )
...
class DepthwiseCondConv2D(tf.keras.layers.DepthwiseConv2D):
    ...
    def build(self, input_shape):
        ...
        depthwise_condconv_kernel_shape = (
            self.num_experts,
    ...
    def call(self, inputs, routing_weights):
        ...
        depthwise_kernels = tf.matmul(routing_weights, self.depthwise_condconv_kernel)
        batch_size = inputs.shape[0].value
        inputs = tf.split(inputs, batch_size, 0)
        ...
        for input_tensor, depthwise_kernel in zip(inputs, depthwise_kernels):
            depthwise_kernel = tf.reshape(depthwise_kernel, self.depthwise_kernel_shape)
            if self.data_format == "channels_first":
