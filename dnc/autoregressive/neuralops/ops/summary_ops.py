...
__all__ = ["scalar_summary", "filters_summary", "images_summary", "gradients_summary"]
...
def scalar_summary(name, value, sess=None, summary_writer=None, step=None):
    ...
    collection = collection_to_dict(_ops.get_collection(_ops.GraphKeys.SUMMARIES))
    # tensorflow tensor
...
def filters_summary(kernel, rescale=True, name="kernel"):
    ...
    with tf.variable_scope("filters_visualization"), tf.device("/cpu:0"):
        ...
        # display all filters
        tf.summary.image(name, kernel_transposed, max_outputs=shape[-1])
...
def images_summary(images, name, max_outs, color_format="BGR"):
    ...
    **Note** that images should be scaled to [-1, 1] for 'RGB' or 'BGR',
    ...
    with tf.variable_scope(name), tf.device("/cpu:0"):
        if color_format == "BGR":
            img = tf.clip_by_value((tf.reverse(images, [-1]) + 1.0) * 127.5, 0.0, 255.0)
        elif color_format == "RGB":
            img = tf.clip_by_value((images + 1.0) * 127.5, 0, 255)
        elif color_format == "GREY":
            img = tf.clip_by_value(images * 255.0, 0, 255)
        else:
