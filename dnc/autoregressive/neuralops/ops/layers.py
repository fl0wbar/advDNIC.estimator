...
def moving_average_var(x, decay=0.99, initial_value=0.0, name="moving_average_var"):
    """Moving_average_var.
    """
    ...
    with tf.variable_scope(name):
        if padding == "SYMMETRIC" or padding == "REFELECT":
            x = tf.pad(
                ...
                [
                    [0, 0],
                    [int((ksize[0] - 1) / 2), int((ksize[0] - 1) / 2)],
...
def max_pool(x, ksize=2, stride=2, padding="SAME", name="max_pool"):
    ...
    s = int2list(stride)
    with tf.variable_scope(name):
        return tf.nn.max_pool(x, [1, k[0], k[1], 1], [1, s[0], s[1], 1], padding)
...
def avg_pool(x, ksize=2, stride=2, padding="SAME", name="avg_pool"):
    ...
    s = int2list(stride)
    with tf.variable_scope(name):
        return tf.nn.avg_pool(x, [1, k[0], k[1], 1], [1, s[0], s[1], 1], padding)
...
def bilinear_upsample(x, scale=2):
    ...
    deconv = tf.nn.conv2d_transpose(
        x, weight_var, out_shape, [1, scale, scale, 1], "SAME"
    )
    edge = scale * (scale - 1)
...
def pixel_flow(x, offset, interpolation="bilinear", name="pixel_flow"):
    ...
    def reindex(x, offset):
        ...
        ofs = tf.shape(offset)
        n_add = tf.tile(
            tf.reshape(tf.range(xs[0]), [xs[0], 1, 1, 1]), [1, xs[1], xs[2], 1]
        )
        h_add = tf.tile(
            tf.reshape(tf.range(xs[1]), [1, xs[1], 1, 1]), [xs[0], 1, xs[2], 1]
        )
        w_add = tf.tile(
            tf.reshape(tf.range(xs[2]), [1, 1, xs[2], 1]), [xs[0], xs[1], 1, 1]
        )
    ...
    with tf.variable_scope(name):
        ...
        if interpolation == "bilinear":
            ...
            assert NotImplementedError(
                "interpolation method: {} is not implemented.".format(interpolation)
            )
