...
def gan_log_loss(pos, neg, name="gan_log_loss"):
    ...
    with tf.variable_scope(name):
        ...
        # loss
        d_loss = tf.add(0.5 * d_loss_pos, 0.5 * d_loss_neg)
        scalar_summary("losses/d_loss", d_loss)
        scalar_summary("losses/g_loss", g_loss)
    return g_loss, d_loss


def gan_ls_loss(pos, neg, value=1.0, name="gan_ls_loss"):
    """
    ...
    with tf.variable_scope(name):
        ...
        d_loss = tf.add(0.5 * l2_pos, 0.5 * l2_neg)
...
def gan_hinge_loss(pos, neg, value=1.0, name="gan_hinge_loss"):
    """
    gan with hinge loss:
    ...
    with tf.variable_scope(name):
        ...
        d_loss = tf.add(0.5 * hinge_pos, 0.5 * hinge_neg)
...
def gradients_penalty(x, y, mask=None, norm=1.0):
