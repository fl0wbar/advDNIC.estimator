            ...
            with tf.colocate_with(v):
                ...
                if var_rank > 1:
                    for i, d in enumerate(shape):
                        ...
                        _ = self._get_or_make_slot(
                            v, diag_init, "accumulator_" + str(i), self._name
                        )
                else:
        ...
        with tf.control_dependencies(accumulator_update_ops):
            if self._momentum > 0:
                ...
                update = tf.assign_add(
                    gbar, gbar * (self._momentum_tensor - 1.0) + scaled_g
                )
            else:
                update = scaled_g
    ...
    def _apply_sparse_shared(self, grad_values, grad_indices, var):
        ...
        if var_rank > 1:
            ...
            accumulator = tf.scatter_add(
                accumulator_var, grad_indices, grad_values * grad_values
            )
        ...
        with tf.control_dependencies(updates):
            return tf.scatter_sub(
                var, grad_indices, self._learning_rate_tensor * scaled_g
            )
