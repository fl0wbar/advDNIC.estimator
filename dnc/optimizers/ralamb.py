...
__all__ = ["RLAMBOptimizer"]


class RLAMBOptimizer(optimizer.Optimizer):
    """RAdam with LARS optimizer.
    ...
    - [BERT: Pre-training of Deep Bidirectional Transformers for Language Understanding] https://arxiv.org/abs/1810.04805
        ...
        LARS is Layer-wise Adaptive Rate Scaling for large batch training.
        ...
        use the default RAdam optimizer.
    """
    ...
    def __init__(
        ...
        trust_ratio_coeff=1.0,  # The LARS trust_ratio coefficient is a hyperparameter (keep it 1.0)
        trust_ratio_clip=None,  # The LARS trust_ratio clip is a hyperparameter (keep it None)
        # Enable skipping variables from LARS scaling.
        # TODO : Enable a direct mechanism to pass a subset of variables to the optimizer.
        skip_list=None,
        amsgrad=False,
        total_steps=0,
        ...
        r"""Construct a new Rectified Adam optimizer with LARS.
        Args:
            ...
            trust_ratio_coeff: A scaling factor for the trust ratio in LARS
            trust_ratio_clip: A clipping factor for the trust ratio in LARS
        ...
        """
        super(RLAMBOptimizer, self).__init__(use_locking, name)
        self._lr = learning_rate
        self._beta1 = beta1
        self._beta2 = beta2
        self._eeta = eeta
        self._trust_ratio_coeff = trust_ratio_coeff
    ...
    def _create_slots(self, var_list):
        ...
        self._create_non_slot_variable(
            initial_value=1.0, name="step", colocate_with=first_var
        )
        self._create_non_slot_variable(
    ...
    def _prepare(self):
        ...
        self._warmup_proportion_t = ops.convert_to_tensor(
    ...
    def compute_lr(self, grad, var):
        # LARS (applied only to dense updates)
        scaled_lr = self._lr
        if self._skip_list is None or not any(v in var.name for v in self._skip_list):
            ...
            # Clip the trust ratio (not needed)
            if isinstance(self._trust_ratio_clip, float):
                trust_ratio = min(trust_ratio, self._trust_ratio_clip)
            # Scale the trust ratio
            trust_ratio = self._trust_ratio_coeff * trust_ratio

            # compute the local learning rate
            scaled_lr = self._lr * trust_ratio
        return scaled_lr
    ...
    def _resource_apply_dense(self, grad, var):
        ...
        scaled_lr_t = ops.convert_to_tensor(scaled_lr, name="lars_learning_rate")
        lr_t = math_ops.cast(scaled_lr_t, var.dtype.base_dtype)

        if self._initial_total_steps > 0:
            total_steps = math_ops.cast(self._total_steps_t, var.dtype.base_dtype)
            warmup_proportion = math_ops.cast(
                self._warmup_proportion_t, var.dtype.base_dtype
            )
        ...
        m_t = state_ops.assign(
            m, beta1_t * m + (1.0 - beta1_t) * grad, use_locking=self._use_locking
        )
        ...
        if self._amsgrad:
            ...
            vhat_t = state_ops.assign(
                vhat, math_ops.maximum(vhat, v_t), use_locking=self._use_locking
            )
    ...
    # Fallback to RAdam optimizer (without LARS) for sparse tensors
    ...
    def _finish(self, update_ops, name_scope):
        ...
        return control_flow_ops.group(
            *update_ops + [update_step, update_beta1, update_beta2], name=name_scope
        )
