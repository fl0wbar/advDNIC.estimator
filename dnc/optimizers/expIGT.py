...
def get_gamma_t(step, tail_fraction):
    ...
    gamma_t = c * (s - 1.0) / (1.0 + c * (s - 1.0))
    gamma_t *= 1.0 - tf.sqrt((1 - c) / (s * (s - 1.0))) / c

    return tf.cond(tf.equal(step, 1), lambda: 0.0, lambda: gamma_t)
...
class ExpIgtOptimizer(object):
    ...
    def __init__(
        ...
        if optimizer == "gd":
            ...
            raise ValueError("{} is not a supported apply-optimizer.".format(optimizer))
    ...
    def _create_slots(self, var_list):
        ...
        for var in var_list:
            ...
            true_param = slot_creator.create_slot(
                var, var.initialized_value(), "true_param"
            )
    ...
    def apply_gradients(self, grads_vars, global_step=None, name=None):
        ...
        for g, var in grads_vars:
            ...
            estimate = estimate.assign(gamma_t * estimate + (1.0 - gamma_t) * g)
        ...
        with tf.control_dependencies(clear_deps):
            ...
            apply_op = self._optimizer.apply_gradients(
                apply_optimizer_grads_vars, global_step, name
            )
        ...
        with tf.control_dependencies([apply_op]):
            for g, var in grads_vars:
                ...
                multiplier = next_gamma_t / (1.0 - next_gamma_t)
