    ...
    def _finish(self, state):
        ...
        for var, grad, grad_at_prev_iterate in zip(
            ...
            if self.output_summaries:
                ...
                tf.summary.scalar(
                    self._name + "/max_grad/" + var.name, maximum_gradient_updated
                )
                tf.summary.scalar(
                    self._name + "/average_beta/" + var.name, tf.reduce_mean(beta)
                )
                tf.summary.scalar(
                    self._name + "/iterate_diff/" + var.name,
                    tf.norm(var - previous_iterate),
                )
                tf.summary.scalar(
                    self._name + "/grad_diff/" + var.name,
