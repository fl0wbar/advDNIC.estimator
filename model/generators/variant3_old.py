...
def generator(
    ...
    if is_training:
        ...
        if decompression:
            ...
            side_string_y = decompression_vars["side_string_y"]
            side_string_z = decompression_vars["side_string_z"]
            z_shape = decompression_vars["z_shape"]
            y_shape = decompression_vars["y_shape"]

            with tf.variable_scope("generator"):
                y_hat_cm = entropy_bottleneck.decompress(
                    side_string_y, y_shape, channels=params.num_filters
                )
                z_hat = entropy_bottleneck.decompress(
                    side_string_z, z_shape, channels=params.num_filters
                )
                psi = hyper_decoder(z_hat)
                phi = context_model(y_hat_cm)
                ep_in = tf.concat([phi, psi], axis=3)
                mu, sigma = entropy_parameters(ep_in)
            ...
            with tf.variable_scope("generator"):
                ...
                if predict:
                    ...
                    side_string_y = entropy_bottleneck.compress(y)
                    side_string_z = entropy_bottleneck.compress(z)
