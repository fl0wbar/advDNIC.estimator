...
def decompress(args):
    ...
    params = HParams(json_path)

    # Instantiate model.
    synthesis_transform = SynthesisTransform(params.num_filters)
    hyper_synthesis_transform = HyperSynthesisTransform(params.num_filters)
    entropy_bottleneck = tfc.EntropyBottleneck(dtype=tf.float32)

    # Decompress and transform the image back.
    z_shape = tf.concat([z_shape, [params.num_filters]], axis=0)
